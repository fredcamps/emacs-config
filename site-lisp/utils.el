...
(defcustom utils:kill-all-buffers-hooks (list)
  "Hooks to be used when killing all buffers.  LIST."
  :type 'list
  :group 'utils)


(defun utils:get-home-dir ()
  "Retrieve dir from 'user-init-file'."
...
(defun utils:get-snippets-directory ()
  "Get snippets directory."

  (let (snippets-directory)
    (setq snippets-directory
	  (file-name-as-directory (concat user-emacs-directory "snippets")))))
...
(defun utils:get-parent-dir (dirname-or-filename)
  "Retrieve parent directory.  DIRNAME-OR-FILENAME."
  (defvar parent-dir nil)
  (setq parent-dir
	(file-name-directory (directory-file-name dirname-or-filename)))
